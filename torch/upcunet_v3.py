    ...
    def forward(self, x, tile_mode=0):  # 1.7G
            ...
            for j in range(0, w-36, crop_size[1]):
                ...
                if ("Half" in x.type()):  # torch.HalfTensor/torch.cuda.HalfTensor
                    tmp_se_mean = torch.mean(
                        x_crop.float(), dim=(2, 3), keepdim=True).half()
                else:
        ...
        for i in range(0, h-36, crop_size[0]):
            for j in range(0, w-36, crop_size[1]):
                ...
                if ("Half" in x.type()):  # torch.HalfTensor/torch.cuda.HalfTensor
                    tmp_se_mean = torch.mean(
                        tmp_x2.float(), dim=(2, 3), keepdim=True).half()
                else:
        ...
        for i in range(0, h-36, crop_size[0]):
            for j in range(0, w-36, crop_size[1]):
                ...
                if ("Half" in x.type()):  # torch.HalfTensor/torch.cuda.HalfTensor
                    tmp_se_mean = torch.mean(
                        tmp_x3.float(), dim=(2, 3), keepdim=True).half()
                else:
        ...
        for i in range(0, h-36, crop_size[0]):
            for j in range(0, w-36, crop_size[1]):
                ...
                if ("Half" in x.type()):  # torch.HalfTensor/torch.cuda.HalfTensor
                    tmp_se_mean = torch.mean(
                        tmp_x4.float(), dim=(2, 3), keepdim=True).half()
                else:
